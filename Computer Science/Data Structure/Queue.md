...
### 순차 자료구조
큐를 1차원 배열을 통해 구현되는 방식은 데이터가 순차적으로 쌓이는 순서를 배열의 인덱스로 표현한다. rear 변수에는 마지막 데이터의 인덱스를 저장하여 데이터 삽입이 마지막 위치에서 이루어지도록 구현하고, front 변수에는 첫 번째 데이터의 인덱스를 저장하여 데이터 삭제가 처음 위치에서 이루어지도록 구현한다.  
- 초기 상태 : front = rear = -1
...
### Circular Queue  
원형 큐란, 처음과 끝이 연결된 순환 형태의 자료구조이다.
...
<img src = "../../IMG/CS/DS/queue_insert.png" width = "400">


#### 삭제

<img src = "../../IMG/CS/DS/queue_delete.png" width = "400">
...
### 응용 분야
큐눈 선입선출(FIFO, First In First Out) 구조를 가지고 있기 때문에 발생한 순서대로 문제를 해결해야 하는 작업에서 주로 사용된다.
- 운영체제의 작업 큐
    - 프린터 버퍼 큐 : CPU 작업속도가 출력속도보다 빠른 경우, CPU는 출력이 끝날 때까지 다른 작업을 수행할 수 없는 비효율성 발생한다. 그래서 프린터 버퍼큐를 사용하게 한다. CPU가 출력으로 내보내는 순서대로 프린터 버퍼 큐에서 삽입하면, 프린터는 CPU가 처리한 작업 순서대로 작업을 효율적으로 수행할 수 있다.
    - 스케줄링 큐 : CPU 사용 스케줄링 큐에 삽입하게 되면, 스케줄 순서대로 프로세스를 순서대로 관리할 수 있다.
- 시뮬레이션의 대기 행렬 큐 : 서비스를 받기 위해 기다리는 대기자를 대기 행렬 큐에 삽입하여, 순서대로 시뮬레이션 처리한다. 이처럼 대기 행렬을 모델링하고 처리하는데 큐를 사용할 수 있다.
- 너비 우선 탐색(BFS)
