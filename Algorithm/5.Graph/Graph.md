...
### 3.2 연결 자료구조 방식을 이용한 Garph의 구현 : 인접 리스트(Adjacent List)
    👉 배열과 Linked List로 나열하는 방법  
그래프를 구성하는 정점에 대한 인접 정점들을 오른차순으로 연결리스트에 저장한다.각 정점에 대한 참조변수를 배열로 구성하고, 각 정점에 대한 헤드 노드는 인접 정점의 노드 번호에 대한 오름차순으로 정렬하고 연결한 리스트를 가리킨다. 이렇게 그래프를 표현한 리스트을 인접 리스트(Adjacent List)이라고 한다. 
